...
pub fn verify(allocator: mem.Allocator, pks: []const PublicKey, bytes: []const u8, sig: Signature, prehash: ?bool) !void {
    var i: usize = pks.len;
    while (i > 0) {
        i -= 1;

        if (pks[i].verifySlice(allocator, bytes, sig, prehash)) |_| {
            return;
        } else |_| {}
    }
